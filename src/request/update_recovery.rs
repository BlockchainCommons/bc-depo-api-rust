...
impl From<UpdateRecovery> for Expression {
    fn from(value: UpdateRecovery) -> Self {
        ...
        Expression::new(UPDATE_RECOVERY_FUNCTION).with_parameter(RECOVERY_METHOD_PARAM, method)
    }
}
...
impl TryFrom<Expression> for UpdateRecovery {
    ...
    fn try_from(expression: Expression) -> Result<Self> {
        let recovery_object = expression.object_for_parameter(RECOVERY_METHOD_PARAM)?;
...
impl std::fmt::Display for UpdateRecovery {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(
            format_args!("{} to {}", "updateRecovery".flanked_function(), self.recovery().abbrev())
        )
    }
}
